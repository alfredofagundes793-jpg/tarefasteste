...
function handleMouseDown(e) {
  ...
  if (!taskItem || e.button !== 0) return;
  
  // Limpar elementos fantasma antigos
  document.querySelectorAll('.drag-ghost').forEach(el => el.remove());
  
  dragState.draggedElement = taskItem;
  dragState.draggedTaskId = taskItem.dataset.taskId;
  dragState.draggedListId = taskItem.dataset.listId;
  dragState.draggedSublistId = taskItem.dataset.sublistId;
  dragState.sourceScreen = currentScreen;
  ...
  taskItem.classList.add('dragging-source');
}
...
function handleMouseMove(e) {
  ...
  // Só iniciar drag se mover mais de 5px
  if (moveDistance < 5) return;
  
  if (!dragState.isDragging) {
    dragState.isDragging = true;
    createGhostElement(dragState.draggedElement, e.clientX, e.clientY);
  }
  ...
  dragState.ghostElement.style.pointerEvents = 'auto';
  
  const targetItem = elementBelow?.closest('.task-item, .kanban-card, .calendar-task-item, .calendar-week-task-item');
  ...
  if (targetItem && targetItem !== dragState.draggedElement) {
    ...
    if (e.clientY < midpoint) {
      targetItem.classList.add('drag-indicator-top');
    } else {
      targetItem.classList.add('drag-indicator-bottom');
    }
  }
...
function handleMouseUp(e) {
  ...
  if (dragState.isDragging) {
    ...
    if (targetItem && targetItem !== dragState.draggedElement && dragState.draggedTaskId) {
      const targetTaskId = targetItem.dataset.taskId;
      const targetListId = targetItem.dataset.listId;
      const targetSublistId = targetItem.dataset.sublistId;
      
      if (dragState.draggedListId === targetListId && dragState.draggedSublistId === targetSublistId) {
        const sublist = taskManager.getSublist(dragState.draggedListId, dragState.draggedSublistId);
        if (sublist) {
          const sortedTasks = taskManager.sortTasks(sublist.tasks);
          const targetIndex = sortedTasks.findIndex(t => t.id === targetTaskId);
          
          if (targetIndex !== -1) {
            taskManager.reorderTask(dragState.draggedListId, dragState.draggedSublistId, dragState.draggedTaskId, targetIndex);
  ...
  // Limpar tudo
  document.querySelectorAll('.dragging-source, .drag-indicator-top, .drag-indicator-bottom').forEach(el => {
    el.classList.remove('dragging-source', 'drag-indicator-top', 'drag-indicator-bottom');
  });
  
  dragState.isDragging = false;
  dragState.draggedElement = null;
  dragState.draggedTaskId = null;
  dragState.draggedListId = null;
  dragState.draggedSublistId = null;
  dragState.ghostElement = null;
}
...
function handleTouchStart(e) {
  ...
  createGhostElement(taskItem, touch.clientX, touch.clientY);
  taskItem.classList.add('dragging-source');
}

function handleTouchMove(e) {
  if (!touchState.element || !touchState.ghostElement) return;
  
  const touch = e.touches[0];
  
  touchState.ghostElement.style.left = (touch.clientX - touchState.offsetX) + 'px';
  touchState.ghostElement.style.top = (touch.clientY - touchState.offsetY) + 'px';
  
  touchState.ghostElement.style.pointerEvents = 'none';
  const element = document.elementFromPoint(touch.clientX, touch.clientY);
  touchState.ghostElement.style.pointerEvents = 'auto';
  
  const targetItem = element?.closest('.task-item, .kanban-card, .calendar-task-item, .calendar-week-task-item');
  
  document.querySelectorAll('.drag-indicator-top, .drag-indicator-bottom').forEach(el => {
    el.classList.remove('drag-indicator-top', 'drag-indicator-bottom');
  });
  
  if (targetItem && targetItem !== touchState.element) {
    const rect = targetItem.getBoundingClientRect();
    const midpoint = rect.top + rect.height / 2;
...
function handleTouchEnd(e) {
  ...
  if (targetItem && targetItem !== touchState.element) {
    const draggedTaskId = touchState.element.dataset.taskId;
    const draggedListId = touchState.element.dataset.listId;
    const draggedSublistId = touchState.element.dataset.sublistId;
    
    const targetTaskId = targetItem.dataset.taskId;
    const targetListId = targetItem.dataset.listId;
    const targetSublistId = targetItem.dataset.sublistId;
    
    if (draggedListId === targetListId && draggedSublistId === targetSublistId) {
      const sublist = taskManager.getSublist(draggedListId, draggedSublistId);
      if (sublist) {
        const sortedTasks = taskManager.sortTasks(sublist.tasks);
        const targetIndex = sortedTasks.findIndex(t => t.id === targetTaskId);
        
        if (targetIndex !== -1) {
          taskManager.reorderTask(draggedListId, draggedSublistId, draggedTaskId, targetIndex);
          refreshCurrentScreen();
        }
      }
    }
