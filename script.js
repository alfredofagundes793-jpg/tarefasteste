/* ============================================================================
   ...
   Gerenciamento de tarefas com suporte a horários e visualização de calendário
   ============================================================================ */

// ============================================================================
// Classe TaskManager - Gerenciamento de Dados
// ============================================================================
...
class TaskManager {
  ...
  saveToStorage() {
    localStorage.setItem('taskmaster-data', JSON.stringify(this.lists));
  }

  loadFromStorage() {
    const data = localStorage.getItem('taskmaster-data');
    if (data) {
  ...
  createTask(listId, sublistId, title) {
    ...
    if (sublist) {
      const task = {
        ...
        status: 'todo',
        dueDate: '',
        time: '', // NOVO: Campo de horário (formato HH:MM)
        note: '',
        flagged: false,
        completed: false,
        createdAt: new Date().toISOString()
  ...
  // Obter todas as tarefas ativas ordenadas
  ...
  // NOVO: Obter tarefas por data (para calendário)
  ...
  // NOVO: Obter todas as tarefas com data definida
  getAllTasksWithDate() {
    const allTasks = [];
    this.lists.forEach(list => {
      list.sublists.forEach(sublist => {
        sublist.tasks.forEach(task => {
          if (task.dueDate && task.status !== 'done' && !task.completed) {
            allTasks.push({
              ...task,
              listId: list.id,
              listName: list.name,
  ...
  if (tasks.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-text">Crie sua primeira tarefa nesta sublista</div>
      </div>
    `;
  } else {
    tasks.forEach(task => {
...
function createTaskElement(task, listId, sublistId) {
  ...
  if (task.time) {
    metaHtml += `<span class="task-time">⏰ ${task.time}</span>`;
  }
  ...
  taskEl.innerHTML = `
    ...
    <div class="task-actions">
      <button class="btn-icon" onclick="event.stopPropagation(); deleteTask('${listId}', '${sublistId}', '${task.id}')">🗑️</button>
    </div>
  `;
...
function showCreateTaskModal(listId, sublistId) {
  ...
  document.getElementById('task-input-time').value = ''; // NOVO: Limpar campo de horário
...
function saveTask() {
  ...
  const time = document.getElementById('task-input-time').value; // NOVO: Obter horário
  const note = document.getElementById('task-input-note').value.trim();
  ...
  const updates = {
    ...
    priority: priority,
    dueDate: dueDate,
    time: time, // NOVO: Incluir horário
    note: note,
    flagged: flagged,
    status: status
  };
...
function refreshCurrentScreen() {
  ...
  else if (currentScreen === 'calendar') renderCalendarScreen(); // NOVO
}
...
function editTask(listId, sublistId, taskId) {
  ...
  document.getElementById('task-input-time').value = task.time || ''; // NOVO: Carregar horário
...
// ============================================================================
// NOVA TELA: Calendário
// ============================================================================

let calendarCurrentDate = new Date();
let calendarViewMode = 'day'; // 'day', 'week', 'month'
...
function renderCalendarScreen() {
  ...
  controls.innerHTML = `
    <button class="calendar-btn ${calendarViewMode === 'day' ? 'active' : ''}" onclick="switchCalendarView('day')">Dia</button>
    <button class="calendar-btn ${calendarViewMode === 'week' ? 'active' : ''}" onclick="switchCalendarView('week')">Semana</button>
    <button class="calendar-btn ${calendarViewMode === 'month' ? 'active' : ''}" onclick="switchCalendarView('month')">Mês</button>
  `;
  ...
  nav.innerHTML = `
    <button class="calendar-nav-btn" onclick="calendarPrevious()">← Anterior</button>
    <span class="calendar-date-display" id="calendar-date-display"></span>
    <button class="calendar-nav-btn" onclick="calendarNext()">Próximo →</button>
    <button class="calendar-nav-btn calendar-today-btn" onclick="calendarToday()">Hoje</button>
  `;
...
function renderCalendarDay(container) {
  const dateStr = formatDateForStorage(calendarCurrentDate);
  document.getElementById('calendar-date-display').textContent = formatDateForDisplay(calendarCurrentDate);

  const tasks = taskManager.getTasksByDate(dateStr);

  // Separar tarefas sem horário e com horário
  const tasksWithoutTime = tasks.filter(t => !t.time);
  const tasksWithTime = tasks.filter(t => t.time).sort((a, b) => a.time.localeCompare(b.time));
  ...
  if (tasksWithoutTime.length > 0) {
    ...
    noTimeSection.innerHTML = '<div class="calendar-section-title">Sem Horário</div>';
    ...
    dayContainer.appendChild(noTimeSection);
  }
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    hourDiv.className = 'calendar-hour-slot';
    ...
    const tasksInHour = tasksWithTime.filter(t => t.time.startsWith(String(hour).padStart(2, '0')));
    const tasksContainer = document.createElement('div');
    tasksContainer.className = 'calendar-tasks-in-hour';
  ...
  if (tasks.length === 0) {
    dayContainer.innerHTML = `
      <div class="empty-state">
        <div class="empty-state-icon">📅</div>
        <div class="empty-state-title">Nenhuma tarefa agendada</div>
        <div class="empty-state-text">Nenhuma tarefa para este dia</div>
      </div>
    `;
  }
...
function renderCalendarWeek(container) {
  // Próximos 7 dias a partir da data atual
  const weekDays = [];
  for (let i = 0; i < 7; i++) {
  ...
  const startFormatted = formatDateForDisplay(weekDays[0]).split(' ').slice(1).join(' ');
  const endFormatted = formatDateForDisplay(weekDays[6]).split(' ').slice(1).join(' ');
  document.getElementById('calendar-date-display').textContent = `${startFormatted} até ${endFormatted}`;
  ...
  headerContainer.className = 'calendar-week-header';

  weekDays.forEach(day => {
    const dayHeader = document.createElement('div');
    dayHeader.className = 'calendar-week-day-header';
    const isToday = new Date().toDateString() === day.toDateString();
    dayHeader.style.backgroundColor = isToday ? '#FFF0E5' : 'var(--color-gray-light)';
    dayHeader.style.border = isToday ? '2px solid var(--color-coral)' : '1px solid var(--color-gray-medium)';
    dayHeader.innerHTML = `
      <div class="calendar-week-day-name">${day.toLocaleDateString('pt-BR', { weekday: 'short' })}</div>
      <div class="calendar-week-day-date">${day.getDate()}</div>
    `;
    headerContainer.appendChild(dayHeader);
  });

  weekContainer.appendChild(headerContainer);

  // Colunas de dias
  const columnsContainer = document.createElement('div');
  columnsContainer.className = 'calendar-week-columns';
  ...
  weekDays.forEach(day => {
    ...
    const isToday = new Date().toDateString() === day.toDateString();
    if (isToday) {
      dayColumn.style.backgroundColor = '#FFF0E5';
      dayColumn.style.border = '2px solid var(--color-coral)';
    }

    // Tarefas sem horário
    if (tasksWithoutTime.length > 0) {
      const noTimeSection = document.createElement('div');
      noTimeSection.className = 'calendar-week-no-time-section';
      noTimeSection.innerHTML = '<div class="calendar-week-section-title">S.H.</div>';

      tasksWithoutTime.forEach(task => {
        const taskEl = createCalendarTaskElement(task, true);
        noTimeSection.appendChild(taskEl);
      });

      dayColumn.appendChild(noTimeSection);
    }

    // Timeline
    const timelineContainer = document.createElement('div');
    timelineContainer.className = 'calendar-week-timeline';

    for (let hour = 0; hour < 24; hour++) {
      const hourDiv = document.createElement('div');
      hourDiv.className = 'calendar-week-hour-slot';
  ...
  weekContainer.appendChild(columnsContainer);
...
function renderCalendarMonth(container) {
  ...
  document.getElementById('calendar-date-display').textContent = calendarCurrentDate.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
  ...
  const headerContainer = document.createElement('div');
  headerContainer.className = 'calendar-month-header';
  ...
  monthContainer.appendChild(headerContainer);
  ...
  gridContainer.className = 'calendar-month-grid';

  // Dias do mês anterior
  for (let i = firstDay - 1; i >= 0; i--) {
    const dayCell = document.createElement('div');
    dayCell.className = 'calendar-month-day calendar-month-day-other';
    dayCell.innerHTML = `<div class="calendar-month-day-number">${daysInPrevMonth - i}</div>`;
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (isToday) {
      dayCell.classList.add('calendar-month-day-today');
    }

    let dayContent = `<div class="calendar-month-day-number">${day}</div>`;

    if (tasks.length > 0) {
      dayContent += `<div class="calendar-month-day-tasks">`;
      tasks.slice(0, 3).forEach(task => {
        dayContent += `<div class="calendar-month-task-badge" title="${escapeHtml(task.title)}">${task.time ? task.time : '•'}</div>`;
      });
      if (tasks.length > 3) {
        dayContent += `<div class="calendar-month-task-more">+${tasks.length - 3}</div>`;
      }
      dayContent += `</div>`;
    }
  ...
  for (let day = 1; day <= remainingCells; day++) {
    const dayCell = document.createElement('div');
    dayCell.className = 'calendar-month-day calendar-month-day-other';
    dayCell.innerHTML = `<div class="calendar-month-day-number">${day}</div>`;
    gridContainer.appendChild(dayCell);
  }
...
function createCalendarTaskElement(task, compact = false) {
  const taskEl = document.createElement('div');
  taskEl.className = `calendar-task-item ${compact ? 'compact' : ''}`;
  taskEl.style.cursor = 'pointer';

  const timeDisplay = task.time ? `<span class="calendar-task-time">${task.time}</span>` : '';
  const titleDisplay = compact ? escapeHtml(task.title).substring(0, 20) : escapeHtml(task.title);

  taskEl.innerHTML = `
    <div class="calendar-task-content">
      ${timeDisplay}
      <span class="calendar-task-title">${titleDisplay}</span>
    </div>
  `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.nav-button').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      else if (screen === 'calendar') renderCalendarScreen(); // NOVO
      showScreen(screen);
    });
  });
